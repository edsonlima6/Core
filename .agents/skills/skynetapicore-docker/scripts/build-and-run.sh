...
if command -v docker >/dev/null 2>&1; then
    for tag in $(docker images "skynetapicore" --format "{{.Tag}}" 2>/dev/null || true); do
...
IMAGE_TAG="${IMAGE_TAG:-skynetapibart.azurecr.io/skynetapibart:dev-${TAG_SUFFIX}}"
ALTERNATE_TAG="${ALTERNATE_TAG:-skynetapibart.azurecr.io/skynetapibart/dev:${TAG_SUFFIX}}"
LOCAL_TAG="${LOCAL_TAG:-skynetapicore:${TAG_SUFFIX}}"
PORT="${PORT:-5000}"
CONTAINER_NAME="${CONTAINER_NAME:-skynetapicore}"
SKIP_DOTNET_BUILD="${SKIP_DOTNET_BUILD:-false}"

echo "=========================================================="
echo " SkyNetApiCore Docker Build & Run Workflow (${TAG_SUFFIX})"
echo "=========================================================="

# 1. Build .NET Project locally
if [ "$SKIP_DOTNET_BUILD" != "true" ]; then
    echo -e "\n[Step 1/5] Compiling SkyNetApiCore (Release)..."
    dotnet build SkyNetApiCore/SkyNetApiCore.csproj -c Release
fi

# 2. Build Docker Image
echo -e "\n[Step 2/5] Building Docker image from SkyNetApiCore/Dockerfile..."
echo "  -> Local Tag: ${LOCAL_TAG}"
docker build -f SkyNetApiCore/Dockerfile -t "${LOCAL_TAG}" .
...
echo "=========================================================="
echo " SkyNetApiCore (${TAG_SUFFIX}) container is successfully running!"
