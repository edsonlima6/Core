# SkyNetApiCore Docker Workflow & Azure Container Registry Guide

This reference document details the architecture, Docker configuration, Azure Container Registry (ACR) integration, and troubleshooting procedures for containerizing the `SkyNetApiCore` service.

---

## 1. Docker Build Context Architecture

`SkyNetApiCore/Dockerfile` is a multi-stage Dockerfile targeting .NET 10.

- **SDK Build Stage**: `mcr.microsoft.com/dotnet/sdk:10.0`
  - Restores packages centrally via `Directory.Packages.props` and `Core.sln`.
  - Publishes `SkyNetApiCore.csproj` in `Release` mode with `/p:UseAppHost=false`.
- **Runtime Stage**: `mcr.microsoft.com/dotnet/aspnet:10.0`
  ...
  - Sets entrypoint: `["dotnet", "SkyNetApiCore.dll"]`.

### Build Context Rule

Always execute the build command from the **solution root**, never from inside the `SkyNetApiCore` subdirectory:
...
Therefore:
- `skynetapibart.azurecr.io/skynetapibart:dev:v0.0.2` ❌ (*Invalid syntax: two colons - triggers "invalid reference format" error*)
- `skynetapibart.azurecr.io/skynetapibart:dev-v0.0.2`  (*Valid: repository `skynetapibart`, combined tag `dev-v0.0.2`*)
- `skynetapibart.azurecr.io/skynetapibart/dev:v0.0.2`  (*Valid: repository path `skynetapibart/dev`, tag `v0.0.2`*)
- `skynetapibart.azurecr.io/skynetapibart/skynetapicore:v0.0.2`  (*Valid: repository path `skynetapibart/skynetapicore`, tag `v0.0.2`*)
- `skynetapibart.azurecr.io/skynetapicore:v0.0.2`  (*Valid: repository `skynetapicore`, tag `v0.0.2`*)
...
docker push skynetapibart.azurecr.io/skynetapibart:dev-v0.0.2
```
...
docker logs -f skynetapicore

# Stop and remove container
docker stop skynetapicore
docker rm skynetapicore

# Open interactive shell inside container
docker exec -it skynetapicore /bin/sh
```
