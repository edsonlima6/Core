...
.EXECTUE Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass 

.SYNOPSIS
    Builds the skynetproductsapi project, builds its Docker image, tags it for ACR with automatic version bumping, and runs the container.
...
.PARAMETER LocalTag
    Optional custom local tag override. Defaults to 'skynetproductsapi:v<Version>'.

.PARAMETER Port
    Host port mapped to container port 80. Default is 5000.

.PARAMETER ContainerName
    Name of the running Docker container. Default is 'skynetproductsapi'.
...
param(
    ...
    [string]$LocalTag,
    [int]$Port = 5001,
    [string]$ContainerName = "skynetproductsapi",
...
if ($PSScriptRoot) {
    ...
    $ScriptDir = Join-Path (Get-Location) ".agents/skills/skynetproductsapi-docker/scripts"
}
...
$CandidatePaths = @(
    ...
    (Join-Path (Get-Location) ".agents/skills/skynetproductsapi-docker/VERSION")
)
...
try {
    $dockerTags = docker images "skynetproductsapi" --format "{{.Tag}}" 2>$null
...
if (-not $LocalTag) {
    $LocalTag = "skynetproductsapi:$tagSuffix"
}
if (-not $ImageTag) {
    $ImageTag = "skynetapibart.azurecr.io/skynetproductsapi:dev-$tagSuffix"
}
if (-not $AlternateTag) {
    $AlternateTag = "skynetapibart.azurecr.io/skynetproductsapi/dev:$tagSuffix"
}

Write-Host "==========================================================" -ForegroundColor Cyan
Write-Host " SkynetProductApi Docker Build & Run Workflow ($tagSuffix)" -ForegroundColor Cyan
Write-Host "==========================================================" -ForegroundColor Cyan

# 1. Build .NET Project locally
if (-not $SkipDotnetBuild) {
    Write-Host "`n[Step 1/5] Compiling skynetproductsapi (Release)..." -ForegroundColor Yellow
...
Write-Host "`n[Step 2/5] Building Docker image from SkynetProductsApi/Dockerfile..." -ForegroundColor Yellow
Write-Host "  -> Local Tag: $LocalTag"
docker build -f SkynetProductsApi/Dockerfile -t $LocalTag .
if ($LASTEXITCODE -ne 0) {
...
Write-Host " SkynetProductsApi ($tagSuffix) container is successfully running!" -ForegroundColor Green
